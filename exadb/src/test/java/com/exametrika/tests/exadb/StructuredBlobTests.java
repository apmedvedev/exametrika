...
public class StructuredBlobTests {
    ...
    @Test
    public void testObjectStructuredBlobField() {
        testObjectStructuredBlobField(false, 0);
    }

    @Test
    public void testObjectFixedStructuredBlobField() {
        testObjectStructuredBlobField(true, 200);
    }

    private void testObjectStructuredBlobField(boolean fixedRecord, int fixedRecordSize) {
        ...
        NodeSchemaConfiguration nodeConfiguration2 = new TestNodeSchemaConfiguration("node2",
                Arrays.asList(new IndexedNumericFieldSchemaConfiguration("field1", PrimitiveFieldSchemaConfiguration.DataType.INT),
                        new StructuredBlobFieldSchemaConfiguration("field3", "field3", null, "node1", "field2", true, !fixedRecord,
                                ...
                                Arrays.asList(new StructuredBlobIndexSchemaConfiguration("index1", 0, IndexType.BTREE,
                                        ...
                                        new StructuredBlobIndexSchemaConfiguration("index2", 0, IndexType.BTREE,
                                                ...
                                                true, true, "textIndex2")), true, new TestRecordIndexerSchemaConfiguration())));
        ...
        database.transactionSync(new Operation() {
            ...
            public void run(ITransaction transaction) {
                ...
                IDocumentSchema fullTextSchema = dataSchema.findNode("node3").getFullTextSchema();
                result = space.getFullTextIndex().search(Queries.expression("field1", "*:*").toQuery(fullTextSchema), new Sort("field1"), 100);
                assertThat(result.getTotalCount(), is(10));
                for (int i = 0; i < 10; i++)
                    assertThat(result.getTopElements().get(i).get(), is((Object)space.findNodeById(nids[i])));

                field2.clear();
                field3.clear();

                assertThat(field2.getCurrent(), nullValue());
                assertThat(field3.getCurrent(), nullValue());

                assertThat(field2.getRecords().iterator().hasNext(), is(false));
                assertThat(field2.getReverseRecords().iterator().hasNext(), is(false));
                assertThat(field3.getRecords().iterator().hasNext(), is(false));
                assertThat(field3.getReverseRecords().iterator().hasNext(), is(false));
            }
        });
        ...
        database.transactionSync(new Operation() {
            ...
            public void run(ITransaction transaction) {
                ...
                INodeSearchResult result = fullText2.search(Queries.expression("field1", "*:*").toQuery(indexer2.documentSchema), new Sort("field2"), 100);
        ...
        if (fixedRecord) {
            database.transactionSync(new Operation() {
                ...
                public void run(ITransaction transaction) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        field2.set(rids2[i], new TestRecord("test-" + (9 - i), 9 - i));
                        field3.set(rids3[i], new TestRecord("test2-" + (18 - 2 * i), 18 - 2 * i));
                    }
                }
            });
            ...
            database.transactionSync(new Operation() {
                ...
                public void run(ITransaction transaction) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        assertThat(field2.get(rids2[i]), is(new TestRecord("test-" + (9 - i), 9 - i)));
                        assertThat(field3.get(rids3[i]), is(new TestRecord("test2-" + (18 - 2 * i), 18 - 2 * i)));
                    }
                }
            });
        }
