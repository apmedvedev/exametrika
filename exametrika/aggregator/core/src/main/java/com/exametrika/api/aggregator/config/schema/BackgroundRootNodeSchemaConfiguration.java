...
/**
 * The {@link BackgroundRootNodeSchemaConfiguration} represents a configuration of schema of background root stack node.
 *
 * @author Medvedev-A
 * @threadsafety This class and its methods are thread safe.
 */
public class BackgroundRootNodeSchemaConfiguration extends EntryPointNodeSchemaConfiguration {
    public BackgroundRootNodeSchemaConfiguration(String name, String alias, String description, IndexedLocationFieldSchemaConfiguration primaryField,
    ...
    public INodeObject createNode(INode node) {
        return new BackgroundRootNode(node);
    }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof BackgroundRootNodeSchemaConfiguration))
            return false;

        BackgroundRootNodeSchemaConfiguration configuration = (BackgroundRootNodeSchemaConfiguration) o;
        return super.equals(configuration);
    }

    @Override
    public boolean equalsStructured(NodeSchemaConfiguration newSchema) {
        if (!(newSchema instanceof BackgroundRootNodeSchemaConfiguration))
            return false;

        BackgroundRootNodeSchemaConfiguration configuration = (BackgroundRootNodeSchemaConfiguration) newSchema;
    ...
    protected void check(AggregationComponentTypeSchemaConfiguration componentType) {
        Assert.isTrue(componentType.getKind() == Kind.BACKGROUND_ROOT);
    }

    @Override
    protected Class getNodeClass() {
        return BackgroundRootNode.class;
    }
}
