...
/**
 * The {@link StackErrorLogNodeSchemaConfiguration} represents a configuration of schema of stack error log node.
 *
 * @author Medvedev-A
 * @threadsafety This class and its methods are thread safe.
 */
public class StackErrorLogNodeSchemaConfiguration extends StackLogNodeSchemaConfiguration {
    public StackErrorLogNodeSchemaConfiguration(String name, String alias, String description, IndexedLocationFieldSchemaConfiguration primaryField,
    ...
    public INodeObject createNode(INode node) {
        return new StackErrorLogNode(node);
    }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof StackErrorLogNodeSchemaConfiguration))
            return false;

        StackErrorLogNodeSchemaConfiguration configuration = (StackErrorLogNodeSchemaConfiguration) o;
        return super.equals(configuration);
    }

    @Override
    public boolean equalsStructured(NodeSchemaConfiguration newSchema) {
        if (!(newSchema instanceof StackErrorLogNodeSchemaConfiguration))
            return false;

        StackErrorLogNodeSchemaConfiguration configuration = (StackErrorLogNodeSchemaConfiguration) newSchema;
    ...
    protected void check(AggregationComponentTypeSchemaConfiguration componentType) {
        Assert.isTrue(componentType.getKind() == Kind.STACK_ERROR_LOG);
    }

    @Override
    protected Class getNodeClass() {
        return StackErrorLogNode.class;
    }
}
