...
/**
 * The {@link IntermediateExitPointNodeSchemaConfiguration} represents a configuration of schema of intermediate exit point node.
 *
 * @author Medvedev-A
 * @threadsafety This class and its methods are thread safe.
 */
public class IntermediateExitPointNodeSchemaConfiguration extends ExitPointNodeSchemaConfiguration {
    public IntermediateExitPointNodeSchemaConfiguration(String name, String alias, String description,
    ...
    public INodeObject createNode(INode node) {
        return new IntermediateExitPointNode(node);
    }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof IntermediateExitPointNodeSchemaConfiguration))
            return false;

        IntermediateExitPointNodeSchemaConfiguration configuration = (IntermediateExitPointNodeSchemaConfiguration) o;
        return super.equals(configuration);
    }

    @Override
    public boolean equalsStructured(NodeSchemaConfiguration newSchema) {
        if (!(newSchema instanceof IntermediateExitPointNodeSchemaConfiguration))
            return false;

        IntermediateExitPointNodeSchemaConfiguration configuration = (IntermediateExitPointNodeSchemaConfiguration) newSchema;
    ...
    protected void check(AggregationComponentTypeSchemaConfiguration componentType) {
        Assert.isTrue(componentType.getKind() == Kind.INTERMEDIATE_EXIT_POINT);
    }

    @Override
    protected Class getNodeClass() {
        return IntermediateExitPointNode.class;
    }
}
