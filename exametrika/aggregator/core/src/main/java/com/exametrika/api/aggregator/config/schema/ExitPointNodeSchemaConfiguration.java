...
import com.exametrika.impl.aggregator.nodes.ExitPointNode;
import com.exametrika.spi.exadb.objectdb.INodeObject;
import com.exametrika.spi.exadb.objectdb.config.schema.FieldSchemaConfiguration;
import com.exametrika.spi.exadb.objectdb.config.schema.NodeSchemaConfiguration;

/**
 * The {@link ExitPointNodeSchemaConfiguration} represents a configuration of schema of exit point node.
 *
 * @author Medvedev-A
 * @threadsafety This class and its methods are thread safe.
 */
public class ExitPointNodeSchemaConfiguration extends StackNodeSchemaConfiguration {
    public ExitPointNodeSchemaConfiguration(String name, String alias, String description,
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof ExitPointNodeSchemaConfiguration))
            return false;

        ExitPointNodeSchemaConfiguration configuration = (ExitPointNodeSchemaConfiguration) o;
        return super.equals(configuration);
    }

    @Override
    public boolean equalsStructured(NodeSchemaConfiguration newSchema) {
        if (!(newSchema instanceof ExitPointNodeSchemaConfiguration))
            return false;

        ExitPointNodeSchemaConfiguration configuration = (ExitPointNodeSchemaConfiguration) newSchema;
    ...
    protected void check(AggregationComponentTypeSchemaConfiguration componentType) {
        Assert.isTrue(componentType.getKind() == Kind.EXIT_POINT);
    }

    @Override
    protected Class getNodeClass() {
        return ExitPointNode.class;
    }
}
