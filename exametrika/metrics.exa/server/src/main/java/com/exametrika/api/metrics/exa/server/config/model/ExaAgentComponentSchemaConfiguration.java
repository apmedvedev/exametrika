...
import com.exametrika.api.metrics.exa.server.config.schema.ExaAgentComponentNodeSchemaConfiguration;
import com.exametrika.api.metrics.exa.server.config.schema.ExaAgentComponentVersionNodeSchemaConfiguration;
...
/**
 * The {@link ExaAgentComponentSchemaConfiguration} is a exa agent component schema configuration.
 *
 * @author Medvedev-A
 * @threadsafety This class and its methods are thread safe.
 */
public class ExaAgentComponentSchemaConfiguration extends HealthComponentSchemaConfiguration {
    public ExaAgentComponentSchemaConfiguration(String name, Set<ActionSchemaConfiguration> actions,
    ...
    public void buildNodeSchemas(Set<ObjectNodeSchemaConfiguration> nodes) {
        ...
        buildFields(fields);
        nodes.add(new ExaAgentComponentNodeSchemaConfiguration(getName(), getName(), null, fields, this));

        List<FieldSchemaConfiguration> versionFields = new ArrayList<FieldSchemaConfiguration>();
        buildVersionFields(versionFields);
        nodes.add(new ExaAgentComponentVersionNodeSchemaConfiguration(getName() + "Version", getName() + "Version", null, versionFields, this));
    }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof ExaAgentComponentSchemaConfiguration))
            return false;

        ExaAgentComponentSchemaConfiguration configuration = (ExaAgentComponentSchemaConfiguration) o;
        return super.equals(configuration);
    }

    @Override
    public boolean equalsStructured(ComponentSchemaConfiguration newSchema) {
        if (!(newSchema instanceof ExaAgentComponentSchemaConfiguration))
            return false;

        ExaAgentComponentSchemaConfiguration configuration = (ExaAgentComponentSchemaConfiguration) newSchema;
    ...
    protected void buildVersionFields(List<FieldSchemaConfiguration> fields) {
        ...
        fields.add(new SingleReferenceFieldSchemaConfiguration("server", null));
    }
}
